...
\begin{tikzpicture}[remember picture, overlay]
  \node[shift={(-3.8cm,-5.8cm)}] at (current page.center)
...
\begin{itemize}
  ...
  \item[$\bullet$] On the source mesh, the signal is constituted of two von Mises density functions that differ
  by their concentration (large and small).
  \item[$\bullet$] On the target mesh, only the large one is present, but at a different location.
\end{itemize}
\vspace{0.2cm}
\begin{figure}
  \centering
...
\begin{itemize}
  ...
  \item[$\bullet$] Source features = antibodies, target features = gene expressions.
\end{itemize}
\vspace{-0.5cm}
\begin{figure}
      \centering
