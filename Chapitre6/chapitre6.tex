...
For any $t \in \bbR$ such that $tp \in \text{dom}(\varphi)$, we have
\begin{align}
  D_{\varphi}(t p | q)
  &= \varphi(tp) - \varphi(q) - \langle \nabla\varphi(q), tp - q \rangle \\
  &= \varphi(tp) - \varphi(q) - \Big[
    t \langle \nabla \varphi(q), p - q \rangle + (t-1) \langle \nabla \varphi(q), q \rangle
  \Big] \\
  &= \varphi(tp) - \varphi(q) + t \Big[ D_{\varphi}(p | q) + \varphi(q) - \varphi(p) \Big]
  - (t-1) \langle \nabla \varphi(q), q \rangle \\
  &= t D_{\varphi}(p | q) + \big[ \varphi(tp) - t \varphi(p) \big]
  + (t-1) \big[ \varphi(q) - \langle \nabla \varphi(q), q \rangle \big].
\end{align}
...
\begin{enumerate}
  \item If $D_{\varphi}$ is squared $l_2$-norm, then $\varphi(tP) = t^2 \; \varphi(P)$. So,
  $\langle \nabla \varphi(q), q \rangle - \varphi(q) = \varphi(q)$ and
  $\varphi(p) - \frac{\partial \varphi(tp)}{\partial t} \Big|_{t=1} = -\varphi(p)$.

  \item If $D_{\varphi}$ is the KL divergences, then
  $\varphi(tP) = t \; \varphi(P) + t \log t \; m(P) - t + 1$.
  So, $\langle \nabla \varphi(q), q \rangle - \varphi(q) = m(q) - 1$ and
  $\varphi(p) - \frac{\partial \varphi(tp)}{\partial t} \Big|_{t=1} = 1 - m(p)$.
\end{enumerate}
\end{proof}
