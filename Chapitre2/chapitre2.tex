...
Second, the distance aggregates the features, thus incurs information loss. This can be problematic,
for example, in the high-dimensional setting, where the Euclidean distance is usually not a good
metric
...
In what follows, we denote by $\Delta_{n}=\{w \in \bbR_{> 0}^n: \sum_{i=1}^{n} w_{i}=1\}$
the simplex histogram with $n$ bins. We call $\cX = (X, \mu_1^X, \mu_2^X)$ a \textit{weighted matrix}
defined by a triplet comprised of a matrix $X \in \bbR^{n \times d}$
equipped with the histograms $\mu_1^X \in \Delta_n$ and $\mu_2^X \in \Delta_d$
...
\begin{table}[h]
    ...
    \hline
    OT, UOT & \makecell[l]{Matrices with the same \\ number of columns}
    & Only requires histogram on rows \\
    \hline
    GW, FGW, UGW & Square matrices & \makecell[l]{Histograms on rows and columns \\ must be the same} \\
    \hline
...
\begin{definition}
  A square matrix $A \in \bbR^{n \times n}$ is conditionally positive (or negative) semi-definite
...
\end{proposition}
Roughly speaking, if the similarity matrices is "approximately" conditionally positive
(or negative) semi-definite, then one can safely remove the equality constraint
without changing the minimum. Thus, \Cref{prop:coot_gw_equiv} justifies the rationale behind
...
Approximation. For a detailed discussion on the subtle distinction amongst these algorithms, see
