...
\section{Background on CO-Optimal Transport}
...
independent KL-penalization. Indeed, given any $\pi_k \in U(\mu_k^X, \mu_k^Y)$, for $k=1,2$, we have
\begin{equation}
...
\begin{itemize}
  \item[$\bullet$] When $p=2$ and $c_X, c_Y$ are Euclidean distances (\textit{i.e.} $q=1$),
  ...
  (\textit{i.e.} $q=2$), the upper bound becomes
...
been successfully applied. A natural extension of classic OT, in which the admissible transport plan (a.k.a coupling) can have more
...
inner product. The generalized Kullback-Leibler divergence between two positive vectors
$p, q \in \bbR^n_{> 0}$ is defined as
$\kl(p | q) = \sum_i p_i \log \frac{p_i}{q_i} - \sum_i p_i + \sum_i q_i$.
...
two tensors, the problem \eqref{factor_mmot} is equivalent to a variation of low non-negative rank OT problem (see Appendix for a proof).

As for the existence of the solution to this problem, we have that $U_{\cT}$ is compact because it is a close subset of the
compact set $U(\mu)$, which implies that the problem \eqref{factor_mmot} always admits a solution. Furthermore, observe that
\begin{equation}
  \begin{split}
...
A special case of Problem \eqref{relax_mmot} is when $M = N$, we recover the entropic-regularized MMOT problem, up to a constant.
...
\citep{Le18}} \citep{Tao86,Tao97} as follows: at the iteration $t$,
\begin{enumerate}
...
is not surprising for a non-convex problem. To accelerate the algorithm for small $\varepsilon$,
...
$\mmotdc_{\varepsilon_0}(\cT, \mu)$ can be estimated quickly and accurately using the initialization $P^{(0)}$. Then we solve each
successive problem $\mmotdc_{\varepsilon_n}(\cT, \mu)$ using the previous solution $P_{\varepsilon_{n-1}}$ as initialization. Finally,
the problem $\mmotdc_{\varepsilon}(\cT, \mu)$ is solved using the solution $P_{\varepsilon_N}$ as initialization.
...
Can be used to study the sample complexity of COOT and GW distances.
\begin{equation}
  | \coot(\cX, \cY) - \coot(\cX_n, \cY_n) |
\end{equation}
The idea is as follows: under some mild (practical) conditions, COOT and GW are equivalent.
So, we prefer COOT to GW as it is less (unnecessarily) constrained.
Inspired by the DC algo (or also convex-concave procedure),
we linearize COOT and construct a "nice" lower bound, which:
\begin{enumerate}
  \item Is an entropic MMOT problem (up to a constant). In this case, one can reuse the technique
  used to study sample complexity of entropic OT in \citep{Genevay19} and extend to the entropic MMOT.
  \item Approaches COOT for suitable choice of inputs.
\end{enumerate}
\begin{definition}
  For each $\pi \in \cU_4$, denote
  $G(\pi) := \kl(\pi_{\# 12} | \mu) + \kl(\pi_{\# 34} | \mu)
  = \kl(\pi_{\#} | \mu^{\otimes 2})$.
\end{definition}
\begin{lemma}
  $G$ is convex and differentiable. In particular, for any $\pi \in \mathcal{U}_4$, we have
  $G(\pi) = \langle \nabla G(\pi), \pi \rangle$.
\end{lemma}
Now, we have
\begin{align}
  \langle c, \gamma \rangle + \varepsilon \kl(\gamma | \gamma_{\#}) &\leq
  \langle c, \gamma \rangle + \varepsilon \kl(\gamma | \mu^{\otimes 2})
  - \varepsilon \Big[ G(\pi) + \langle \nabla G(\pi), \gamma - \pi \rangle \Big] \\
  &= \langle c - \varepsilon \nabla G(\pi), \gamma \rangle
  + \varepsilon \kl(\gamma | \mu^{\otimes 2})
\end{align}
So,
\begin{align}
  \mmotdc_{\varepsilon} \leq
  \inf_{\gamma \in \cU_4} \langle c - \varepsilon \nabla G(\pi), \gamma \rangle
  + \varepsilon \kl(\gamma | \mu^{\otimes 2})
\end{align}
Now replace $G(\gamma)$ by its linear approximation
$G(\pi) + \langle \nabla G(\pi), \gamma - \pi \rangle = \langle \nabla G(\pi), \gamma \rangle$,
\begin{definition}
  (Surrogate function) For $\gamma, \pi \in \cU_4$, define
  \begin{equation}
      \begin{split}
          F_{\varepsilon}(\gamma, \pi)
          &:= \langle c, \gamma \rangle + \varepsilon \kl(\gamma | \mu^{\otimes 2})
          - \varepsilon \Big[ G(\pi) + \langle \nabla G(\pi), \gamma - \pi \rangle \Big] \\
          &= \langle c - \varepsilon \nabla G(\pi), \gamma \rangle
          + \varepsilon \kl(\gamma | \mu^{\otimes 2})
  ...
  \begin{equation}
      \begin{split}
          L_{\varepsilon}(\pi) &:= \inf_{\gamma \in \cU_4} F_{\varepsilon}(\gamma, \pi)
