...
up to permutation of the matrix coordinates. Notably, $\coot(\cX, \cY) = 0$
if and only if there exist two permutations $\sigma_r \in \mathbb S_{n_x}$ and
$\sigma_c \in \mathbb S_{d_x}$ such that $X_{i,k} = Y_{\sigma_r(i), \sigma_c(k)}$,
for every $i \in [n_x], k \in [d_x]$. We note that the formulation \eqref{eq:discrete_coot}
...
\begin{table}[h]
    ...
    other OT-based divergences. UOT and UGW are the unbalanced OT and unbalanced GW divergence, respectively.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Continuous Co-Optimal Transport} \label{sec:continuous_coot}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
presentation is different to that of \citet{Chowdhury21b}, we still come up with the
...
%   Given a measure network $\cX = (X, c_X, \mu_X)$, we assume that
%   \begin{enumerate}
...
% \end{proposition}
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% The above result is similar to the one known in entropic OT .
...
% Here, we rediscover the \Cref{prop:coot_gw_equiv}, a weaker case where this equality still holds.
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \begin{corollary} \label{kernel_gw_coot}
%   If $L(C_x, C_y)$ defines a conditionally negative semi-definite kernel on $(\cX \times \cY)^2$, then we have the equality
%   between GW distance and COOT. Furthermore, if $(Q_s^*,Q_f^*)$ is a solution of the COOT problem, then $Q_s^*$ and $Q_f^*$ are
%   two solutions of the GW problem. In particular, when the semi-definiteness is replaced by
%   the definiteness, we have $Q_s^* = Q_f^*$.
% \end{corollary}
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}[t]
  ...
  \caption[Histograms of difference between true independent marginal matrices and their approximations]{Histograms of difference between true independent marginal matrices and their approximations. We see that the marginal matrices obtained
...
\begin{figure}[t]
	...
	\caption[Scatter plots of MMOT-DC versus other solvers]{Scatter plots of MMOT-DC versus other solvers. In all three plots, the points tend to concentrate around the line $y=x$,
...
\begin{figure}[ht]
  ...
  \caption[Scatter plots of MMOT-DC-v1 versus other solvers]{Scatter plots of MMOT-DC-v1 versus other solvers. In all three plots, the points tend to concentrate around the line $y=x$,
  which indicates the comparable performance of MMOT-DC-v1. On the other hand, the top-right plot shows the clear superiority of EGW-PGD.}
