...
We present the motivation, then focus on the computational and practical aspects,
which are not usually thoroughly discussed in the OT literature. By contrast,
we only briefly introduce the theory since it has been well-studied and can be found easily
in many prior works, which will be precised during the discussion of each topic.

We start with the balanced OT, which quantifies the discrepancy between probability measures,
...
which results in the unbalanced OT. As a byproduct, it also provides more flexibility to
the comparison of measures, not just limited to the probability ones. The second generalization
...
the Monge problem serves as the historical starting point of the Kantorovich problem and
the Hausdorff distance is particularly useful to develop the original intuition of
the Gromov-Wasserstein distance. Then, we briefly present some theoritical properties of the
Wasserstein distance, which are widely used in machine learning applications.
Finally, we discuss in details the numerical and practical aspects of the
...
\end{equation}
Now, we equip each compact subset in $\cC(Z)$ with a Borel probability measure and consider
...
basic and useful properties, which have much impact in machine learning.
...
The Wasserstein distance defines a distance
...
\footnote{More detailed comparison amongst divergences can be found \citep{Gibbs02}.}.
...
Strong duality also holds in a much more general setting (see Theorem 5.10 in \citep{Villani08}).
...
uses the average of distances in the mini-batches as an estimation of the true OT distance.
The "sliced" method \citep{Rabin12,Bonneel15} develops an alternative metric
called \textit{Sliced Wasserstein distance},
which relies on the fact that computing the $1$-D Wasserstein distance boils down to sorting the
...
From a more mathematical viewpoint, the idea relies on the interesting relation
between the minimum and the logsumexp operation, where informally, one has that:
$\min f(x) = \lim_{\varepsilon \to 0^+} -\frac{1}{\varepsilon} \int e^{-\varepsilon f(x)} dx$.
...
the regularizer is the negative entropy $H$ defined by $H(P) = \sum_{i,j} P_{ij} (\log P_{ij} - 1)$.
...
It is already studied in the early 30's by \citep{Schrodinger32},
...
In particular, \citep{Genevay19} rely on the dual problem to show that entropic OT
...
\paragraph{Algorithm} To solve the dual problem \ref{eq:dual_ot}, one can use the
stochastic approach \citep{Genevay16,Seguy18}, which also allows to cope with large-scale problems.
In this thesis, we are interested in the deterministic one, where the most popular solver
is the Sinkhorn algorithm \citep{Sinkhorn67}
\footnote{For the historical perspective, see Remark 4.5 in \citep{Peyre19}.}
(also known as \textit{matrix scaling algorithm}). Denote $K = e^{-C / \varepsilon}$,
...
The OT plan then can be retrieved from the optimal solution $(f^*, g^*)$ by
\begin{align*}
  P^* = (\mu \otimes \nu) \odot \exp \left( \frac{f^* \oplus g^* - C}{\varepsilon} \right).
\end{align*}
...
The computation can also be further speed up by exploiting the structure of the kernel matrix
(see Section 4.3 in \citep{Peyre19}). Second, one only needs to store the dual vectors
(which cost $O(n)$ in memory) and the OT plan (which costs $O(n^2)$ in memory)
...
To avoid the numerical instability, the implementation in the log-domain is recommended
...
\end{enumerate}
To mitigate this side effect, one can use the redundant parametrization strategy
\citep{Chizat18a,Schmitzer19}.
...
initialize the next one. However, it can be very costly due to the amount of subproblems,
...
Third, there are some recent improvements on the Sinkhorn algorithm,
which rely on careful elaboration of the iterates, for example,
the over-relaxation method \citep{Lehmann21,Thibault21}, the screening strategy \citep{Alaya19},
or the greedy approach \citep{Altschuler17,Lin20,Kostic21}.
There exist also other classes of algorithms for the entropic OT, notably
...
\paragraph{Formulation} To define the UOT problem, we first start with
...
\begin{proposition}[Generalization of Lemma 1 in \citep{Fatras21}]
  Let $\mu, \mu_o$ be two nonnegative finite Borel measures with disjoint compact supports.
  For $\alpha \in [0, 1]$, denote $\widetilde{\mu} = \alpha \mu + (1 - \alpha) \mu_o$
...
\end{proposition}
As we can see, the behavior of the outliers is always controlled in UOT.
Thanks to the marginal relaxation, if the outlier is too impactful,
its impact will quickly saturate to $0$, meaning that it will receive no mass from other points.
By contrast, this is not the case for balanced OT, since every point (including outliers)
must be aligned regardless of its cost, due to the marginal constraints.
...
more general entropic UOT formulation: given the hyperparameters
...
So, it is easy to see that the ambiguity of the entropic regularizer is naturally handled in
Problem \ref{eq:discrete_ent_uot}. More precisely,
\begin{enumerate}
  \item Negative entropy corresponds to $\gamma = 1_{m \times n}$,
  since $\kl(P | 1_{m \times n}) = \langle P, \log P \rangle - m(P) + mn = H(P) + mn$.
  \item KL divergence corresponds to $\gamma = \mu \otimes \nu$.
\end{enumerate}
An interesting property of Problem \ref{eq:discrete_ent_uot}, which does not exist in the
balanced OT, is that the minimum can be explicitly expressed as a function of minimizer.
...
to other unbalanced settings, namely the unbalanced Gromov-Wasserstein and
unbalanced Co-Optimal Transport divergences.
Now, we discuss some existing approaches to Problem \ref{eq:discrete_ent_uot}.

\paragraph{Sinkhorn algorithm} The Sinkhorn algorithm can be extended easily to the
unbalanced setting.
...
and can be accelerated using the workarounds in the balanced setting.
...
% \begin{proof}
% Note that we can write the problem \ref{uot_general} as
% \begin{align}
...
to calculate the inter-space distance. Such situations are ubiquitous in practice.
...
such vectorial representation of the nodes may not even exist by default
...
However, it is not clear how such methods can be applied to real-life applications.
...
Now, we will see that the motivation of the Wasserstein distance via Hausdorff distance presented
...
While we are inspired by Chapter 4 in \citep{Memoli11},
we aim to perform a more thorough literature review, including the most recent development and
...
only if $\varphi$ is injective. The strict preservation of distance can also be softened, meaning
that $d_X = O(\varphi^* d_Y)$. This is known as \textit{approximate embedding} \citep{Matousek13},
which has already been studied since the 80's. Notable examples include the famous
...
Two mm-spaces are \textit{isomorphic} (or sometimes called \textit{strongly isomorphic})
...
\begin{enumerate}
  ...
  \item The strong isomorphism is a very strict condition. Indeed, first,
...
Any solver for GW can be easily applied to the FGW. The feature can also be seen as node embedding
and can be learned simultaneous with the alignment matrix, for example in \citep{Xu19}.
Despite the simplicity, FGW has been successfully used in many applications.
It can be easily integrated into many OT-based divergences, for example low-rank GW \citep{Meyer21b},
...
to the UOT problem. We present the derivation of INPUT, then discuss how
...
its convergence analysis remains a challenging open problem and is left to future work.
...
we will only focus on the KL divergence in our case.
\begin{definition}
  ...
  \begin{equation*}
    \min_{x \in E} f(x)
  \end{equation*}
  For learning rate $\eta > 0$, the BPP scheme with respect to the KL divergence reads
  \begin{equation}
    \label{eq:bppa}
    x^{(t+1)} = \argmin_{x \in E} f(x) + \eta \kl(x, x^{(t)}).
  \end{equation}
\end{definition}
When applying to Problem \ref{eq:discrete_ent_uot}, the BPP iteration boils down to solving
\begin{align}
  \label{eq:bpp_uot}
...
The inexact BPP method has already been studied at the same time as the exact one, and
recently attracted some interest in OT.
While it has been applied to the balanced OT \citep{Xie20,Yang22},
...
appears to show strong performance in practice.
...
Despite the simplicity, the convergence analysis of INPUT seems not ...
We note that the techniques used in the balanced OT \citep{Xie20} can not be adapted to
the unbalanced setting, since they rely on the property of the set of admissible couplings,
which is not available in the unbalanced OT. Moreover, their inexact conditions are also
not trivial to verify in practice.

we postulate that could be

\subsection{Illustration on toy example}

\paragraph{Experimental setup}
We consider a synthetic dataset: the source $X$ contains $200$ points forming an ellipse and a square,
assigned with the same uniform probability on both shapes
$\mu = \frac{1}{200} \sum_{i=1}^{200} \delta_{x_i}$. The target $Y$ also contains $200$ points
...
In Problem \ref{eq:discrete_ent_uot}, we choose $\gamma = \mu \otimes \nu$ and
the cost $C(x, y) = || x - y||^2_2$.
...
since it is more simple to implement, yet appears to perform as well as the Sinkhorn-TI.
...
Apart from these $2$ families of methods, we also compare with Majorization-Minimization \texttt{MM},

\paragraph{Results}
We construct $4$ scenarios to verify if INPUT can overcome the limitations of other existing methods.
The first one considers small relaxations and large regularization.
It is an easy test and we expect all methods perform well.
In the second one, we fix the relaxations but choose small regularization
to test the convergence of Sinkhorn-based methods. The third scenario contains fixed regularization
but very large marginal relaxations to test MM. The last one combines small regularization
and very large relaxations.

It is clear that the INPUT family consistently and significantly outperforms other methods,
The distinction becomes even more visible in the regimes where Sinkhorn and MM struggle.
In particular, except the first scenario, INPUT takes much fewer time to reach the
desired tolerance than Sinkhorn-based methods, while attaining a comparable (sometimes better)
estimation of UOT. Even though MM is as fast as INPUT, the quality of its optimum is not
guaranteed, especially in the scenarios 3 and 4.

Within the family of INPUT, combining INPUT with Sinkhorn-TI yields the most efficient algorithm.
Within the Sinkhorn-based family, Sinkhorn-TI shows clear improvement over Sinkhorn,
even though this advantage quickly diminishes for small regularization.

While this experiment is mostly for the proof-of-concept purpose,
the performance of INPUT is very encouraging, which encourages its usage in the applications.
...
In practice, INPUT has two very appealing features. First, it overcomes the limitations of
...
even in the regimes of very small regularization $\varepsilon$
(where Sinkhorn-based methods struggles) or of very large relaxation $\rho_1, \rho_2$
(where MM wrestles). We summarize the applicability of these algorithms in \Cref{t:uot_algo_compare}.
...
\paragraph{Weaknesses of INPUT} There is no free lunch. INPUT also has two potential drawbacks.
First, it requires the recalculation of the cost matrix at the beginning of each BPP iteration,
...
The motivation of this heuristic comes from \Cref{prop:convergence-exact-sppa},
...
\paragraph{Can we adapt INPUT to solve the squared $l_2$-regularized Problem \ref{uot_general} ?}
...
we doubt that it would bring any gain in convergence speed and computational complexity. Indeed,
...
to each BPP iteration empirically helps accelerating the Sinkhorn calculation
and improving the convergence of the algorithm. By contrast,
the case of squared $l_2$-norm does not suffer the same issue,
but rather on the \textbf{relaxation} parameters. So, more regularization does not bring much benefit.