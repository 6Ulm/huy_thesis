...
The general structure for each topic includes the motivation, the theoretical and numerical aspects.
...
Then, we introduce some important theoretical properties of the Wasserstein distance,
...
a subset $R \subset X \times Y$ is a correspondence between $X$ and $Y$ if and only if
\begin{itemize}
...
\end{definition}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
An example of correspondence is illustrated in \Cref{fig:correspondence}.
When $X$ and $Y$ are finite with cardinals $m$ and $n$, respectively,
then all correspondences can be constructed as follows:
choose a matrix $M \in \{0,1 \}^{m \times n}$ such that every row and column
contains at least a value $1$, then the correspondence can be defined as
...
In this case, there is an intimate relation between the correspondence
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.35\textwidth,keepaspectratio]{Chapitre1/figures/correspondence.pdf}
  \caption[Example of correspondence]{Example of correspondence $R$ between two sets $X = \{x_1, x_2, x_3, x_4\}$ and
  $Y = \{y_1, y_2, y_3, y_4 \}$. Here,
  $R = \{(x_1, y_1), (x_1, y_3), (x_2, y_2), (x_3, y_2), (x_4, y_3), (x_4, y_4) \}$.}
  \label{fig:correspondence}
\end{figure}
Denote $\mathcal R(X,Y)$ the collection of all correspondences between $X$ and $Y$,
...
one can replace the correspondence by the admissible coupling and obtain the Wasserstein distance
\begin{align}
...
\begin{figure}[ht]
  ...
  {Process of transforming the Hausdorff distance into the $p$-Wasserstein distance. "W" indicates the \textit{Wassersteinization} process defined by replacing the optimization over the correspondences by over the admissible couplings.
...
The theory of duality plays a crucial role, not only in theoretical study, but also
...
\paragraph{Properties} Under mild conditions of the ground cost and entropy functions,
...
\begin{definition}[Bregman divergence]
  ...
  where $E = \dom(\varphi)$ is a closed convex set in $\bbR^d$.
  \begin{enumerate}
    \item The Bregman divergence $D_{\varphi}: E \times E \to \bbR_{\geq 0}$ is defined as
    $D_{\varphi}(x, y) = \varphi(x) - \varphi(y) - \langle \nabla \varphi(y), x - y \rangle$.
    \item $G_{\varphi}: E \to \bbR$ is defined as
    $G_{\varphi}(x) = \langle \nabla \varphi(x), x \rangle - \varphi(x)$.
  \end{enumerate}
\end{definition}
\begin{corollary}
  ...
  consider the following UOT problem: for $\rho_1, \rho_2, \varepsilon \geq 0$,
  \begin{align}
    \label{eq:uot_bregman}
    \uot_{\varepsilon, \rho} = \inf_{P \in \cD} \; \langle C, P \rangle
    + \rho_1 D_{\varphi_1}(P_{\# 1} | \mu_1) +
    \rho_2 D_{\varphi_2}(P_{\# 2} | \mu_2) + \varepsilon D_{\varphi}(P | \gamma),
  \end{align}
  Suppose Problem \eqref{eq:uot_bregman} admits a solution $P^*$, then we have
  \begin{equation}
    \begin{split}
      \uot_{\varepsilon, \rho} &=
...
Sinkhorn algorithm \citep{Khiem20}. We note that \Cref{prop:uot_minimizer} covers a broad range of situations,
including the unregularized UOT (\ie, $\varepsilon = 0$) and other unbalanced divergences,
namely the squared $l_2$-regularized UOT \eqref{uot_l2} (see \Cref{coro:uot_l2_minimizer}).
Furthermore, it can also be adapted to the unbalanced Gromov-Wasserstein (see Equation \eqref{eq:ugw_minimizer_minimum})
...
of the minimizer, whose coefficients depend only on the input measures (\ie, $\mu, \nu, \gamma$)
...
% Until very recently, \citet{Sebbouh23} propose to learn simultaneously the transport plan and
...
but also more computationally feasible than its peers. For this reason, it has been used extensively in practice,
...
correspondence alignment \citep{Solomon16}, heterogeneous domain adaptation \citep{Yan18},
...
\end{align}
where the infimum is taken over all $R \in \mathcal R(X,Y)$. By replacing the correspondence
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{proof}
If $\cX$ and $\cY$ are weakly isomorphic, then there exists a common mass splitting
...
But this means $\cZ \in \ms(\cY)$. We conclude that $\cX$ and $\cY$ are weakly isomorphic
...
\paragraph{1. Conditional gradient descent} \citet{Vayer19b} directly estimate the GW problem by using
the Frank-Wolfe algorithm, also known as \textit{conditional gradient descent}
...
but there is no theoretical guarantee with their approach.

\paragraph{3. (Inexact) Bregman proximal point} Inspired by the work of \citet{Xie20} on
...
the limit of a decreasing sequence of free energies. Roughly speaking, at each iteration,
...
\paragraph{Fused GW distance} Structured data is an object made of two components:
...
Moreover, thanks to \Cref{prop:uot_minimizer},
...
\end{align}
meaning that the minimum is a \textbf{quadratic} function of the minimizer. See
Appendix \ref{appendix:subsec_uot} for proof.

\citet{Zhang21} introduce the unbalanced bi-directional Gromov-Monge divergence defined as:
