...
The original OT problem was first formulated by \citet{Monge81}. From a mathematical viewpoint,
...
Mathematically speaking, we define the Monge's problem as follows.
...
However, the smaller the regularization, the more slowly the Sinkhorn algorithm converges.
...
\begin{corollary}
  ...
  \end{align}
  where the constraint set $\cC \subset \cD$ satisfies: for any $P \in \cC$ and $t \in \bbR$,
...
\end{itemize}
Since this formulation requires learning two transport maps which push one distribution to the other,
...
To handle the cost tensor, an efficient, yet easy way to implement strategy is to make it
...
\paragraph{3. (Inexact) Bregman proximal point} Inspired by IPOT \citep{Xie20}, \citet{Xu19, Xu19b}
