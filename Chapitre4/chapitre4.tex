...
\chapter[Fused Unbalanced Gromov-Wasserstein]{Fused Unbalanced Gromov-Wasserstein}

\addcontentsline{tot}{chapter}{section}
\chaptermark{\textbf{Fused Unbalanced Gromov-Wasserstein}}

This paper presents the results from the paper \citep{Thual22} and addresses the unbalaced extension
...
Inter-individual variability is a major impediment when trying to draw generalizable conclusions from neuroimaging data collected on groups of subjects.
Current co-registration procedures rely on limited data, and thus lead to very coarse inter-subject alignments.
In this work, we present a novel method for inter-subject alignment based on Optimal Transport, denoted as
Fused Unbalanced Gromov Wasserstein (FUGW). The method aligns cortical surfaces based on the similarity of their functional signatures in response to a variety of stimulation settings, while penalizing large deformations of
individual topographic organization.
We demonstrate that FUGW is well-suited for whole-brain landmark-free alignment. The unbalanced feature allows to deal with the fact that functional areas
vary in size across subjects. Our results show that FUGW alignment significantly increases between-subject correlation of activity for independent functional data, and leads to more precise mapping at the group level.

\section{Introduction}
\label{sec:introduction}

The availability of millimeter or sub-millimeter anatomical or functional brain images has opened new horizons to
...
that can be measured in brain regions (see \Cref{fig:fugw_intro}).
...
%     \label{eq:fugw_loss}
%     \begin{split}
%     \text{L}_{\theta}(P) \enspace \triangleq \enspace
%     &(1 - \alpha) \enspace
%     \tikzmarknode{wass}{
%         \highlight{blue}{
%             $\sum \limits_{\substack{0 \leq i < n\\0 \leq j < p}} || F^s_i - F^t_j||_2^2 P_{i,j}$
%         }
%     }
%     + \alpha \enspace²
%     \tikzmarknode{gw}{
%         \highlight{teal}{
%             $\sum\limits_{\substack{0 \leq i,k < n\\0 \leq j,l < p}} | D^s_{i, k} - D^t_{j, l}|^2 P_{i,j} P_{k,l}$
%         }
%     }\\
%     &+ \rho \medspace \big(
%     \tikzmarknode{unbal}{
...
%     % Label for wasserstein loss
%     \path (wass.north) ++ (0.6,0.8em) node[anchor=south east,color=blue!65] (wasstext){Wasserstein loss $\text{L}_{\text{W}}(P)$};
%     \draw [color=blue!85](wass.north) |- ([xshift=-0.3ex,color=blue!85]wasstext.south west);
%     % Label for gw loss
%     \path (gw.north) ++ (-2.4,2.0em) node[anchor=north west,color=ForestGreen!85] (gwtext){Gromov-Wasserstein loss $\text{L}_{\text{GW}}(P)$};
%     \draw [color=ForestGreen](gw.north) |- ([xshift=-0.3ex,color=ForestGreen]gwtext.south east);
%     % Label for unbalanced loss
%     \path (unbal.north) ++ (-4.8,-2em) node[anchor=north west,color=red!65] (unbaltext){Marginal constraints $\text{L}_{\text{U}}(P)$};
%     \draw [color=red!85](unbal.south) |- ([xshift=-0.3ex,color=red!85]unbaltext.south west);
%     % Label for entropy
%     \path (entr.north) ++ (-2.1,-2em) node[anchor=north west,color=black!85] (entrtext){Entropy};
%     \draw [color=black](entr.south) |- ([xshift=-0.3ex,color=black]entrtext.south west);
% \end{tikzpicture}
% \vspace*{1\baselineskip}

where $\text{L}_{\text{W}}(P)$ matches vertices with similar features,
$\text{L}_{\text{GW}}(P)$ penalizes changes in geometry
and $\text{L}_{\text{U}}(P)$ fosters matching all parts of the source and target distributions.
...
Eventually, we define $\cX^s \triangleq (F^s, D^s, w^s)$ and
$\cX^t \triangleq (F^t, D^t, w^t)$,
...
\paragraph{Approximation of FUGW}
...
\begin{equation}
    \label{eq:lbfugw}
    \begin{split}
        \quad \fugw(\cX^s, \cX^t) = \quad
        \inf_{\substack{P, Q \geq 0 \\ P = Q}} \text{L}_{\theta}(P, Q)
        \quad \geq \inf_{\substack{P, Q \geq 0 \\ m(P) = m(Q)}}
        L_{\theta}(P, Q) \quad \triangleq \quad \text{LB-FUGW} (\cX^s, \cX^t).
    \end{split}
\end{equation}
...
\begin{equation}
    ...
    \begin{split}
        \text{L}_{\theta}(P, Q) \triangleq
        (1 - \alpha) \enspace \text{L}_{\text{W}}(P, Q) + \alpha \enspace \text{L}_{\text{GW}}(P, Q)
        + \rho \enspace \text{L}_{\text{U}}(P, Q) + \varepsilon \enspace E(P, Q),
    \end{split}
\end{equation}
where
\begin{itemize}
    ...
    \item[$\bullet$] $\text{L}_{\text{W}}(P, Q) \triangleq \langle C, \frac{P + Q}{2} \rangle = \frac{1}{2} ( \sum_{i,j} C_{i,j}P_{i,j} + \sum_{i,j} C_{i,j}Q_{i,j})$ \hfill (Wasserstein)

    \item[$\bullet$] $\text{L}_{\gw}(P, Q) \triangleq \langle G , P \otimes Q \rangle = \sum_{i,j,k,l}G_{i,j,k,l}P_{i,j}Q_{k,l}$ \hfill (Gromov-Wasserstein)

    \item[$\bullet$] $\text{L}_{\text{U}}(P, Q) \triangleq \enspace \kl \Big(P_{\# 1} \otimes Q_{\# 1} \vert w^s \otimes w^s \Big) + \enspace \kl \Big(P_{\# 2} \otimes Q_{\# 2} \vert w^t \otimes w^t \Big)$ \hfill (unbalancing)

    \item[$\bullet$] $E(P, Q) \triangleq \kl \Big(P \otimes Q | (w^s \otimes w^t) \otimes (w^s \otimes w^t) \Big)$ \hfill (entropy)
\end{itemize}
In particular, we have $\text{L}_{\theta}(P, P) = \text{L}_{\theta}(P)$,
which is the objective function of FUGW introduced
in Equation 1. It is difficult to study when equality holds between FUGW and its lower bound.
Here, we aim to understand the potential gap between them. First,
let us introduce the following problem
\begin{equation}
  \widetilde{\fugw}(\cX^s, \cX^t) = \inf_{(P, Q) \in \cE} \text{L}_{\theta}(P, Q),
\end{equation}
where we denote $\cE = \{P, Q \geq 0: P_{\#1} = Q_{\#1}, P_{\#2} = Q_{\#2} \}$
...
However, this gap can be tightened (or even closed) in some cases.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{corollary} \label{ugw_uCOOT}
    If the matrices of geodesic distances $D^s$ and $D^t$ satisfy the conditions
    in \Cref{coot_gw_equiv}, then we have
    $\text{LB-FUGW} (\cX^s, \cX^t) = \widetilde{\fugw}(\cX^s, \cX^t)$. Furthermore, if
    the definiteness holds, then the optimal solution satisfies $P^* = Q^*$, meaning that
    $\text{LB-FUGW} (\cX^s, \cX^t) = \fugw(\cX^s, \cX^t)$.
\end{corollary}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In our experiments, while the geodesic distances do not necessarily meet the conditions
in \Cref{coot_gw_equiv}, we still observe that $\text{LB-FUGW} (\cX^s, \cX^t)$ and
$\fugw(\cX^s, \cX^t)$ are numerically equal and that the optimal solution $(P^*, Q^*)$ of
$\text{LB-FUGW} (\cX^s, \cX^t)$ satisfy that $P^*$ and $Q^*$ are also numerically equal.
...
We use the optimal coupling matrix $P$ obtained from Equation \ref{eq:fugw} to transport
...
See Alg. \ref{alg:fugw_barycenter} for more details.
...
in \ref{eq:lbfugw} is differentiable in $F^B$ and $D^B$, and the two couplings of
...
\begin{algorithm}[th]
    \caption{LB-FUGW barycenter for Problem \ref{eq:barycenter}}
...
and derive $F^B$ and $(P^{s,B})_{s \in S}$ from problem \ref{eq:barycenter}.
...
Figure \ref{fig:gain_comparisions_fsaverage5} where correlation grows by almost $40\%$
from $0.258$ to $0.356$.
...
\end{figure}
We also varied training sets by selecting subsets of training contrasts and find that similar performance
on the test set can be achieved regardless of the training data (see Supplementary section \ref{sec:control_experiments} and in particular Supplementary Table \ref{tab:varying_training_sets}).
...
Computation took about 100 hours using 4 Tesla V100-DGXS-32GB GPUs. More precisely, it takes about 4 minutes to compute one coupling between a source and target 10k-vertex hemisphere on a single GPU, when the solver was set to run 10 BCD and 400 Sinkhorn iterations. In comparison, MSM takes about the same time on Intel(R) Xeon(R) CPU E5-2698 v4 @ 2.20GHz CPUs.
Results are reported in Figure \ref{fig:cv_metrics} and provide multiple insights concerning FUGW.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=1.05\columnwidth]{./Chapitre4/figures/cv_all_metrics_hcp_left_fugw.pdf}
    \caption{
...
in size between pairs of individuals. This is illustrated in Figure \ref{fig:transported_mass},
...
As shown in Figure \ref{fig:gain_comparisions_individual},
...
We give an example in Figure \ref{fig:individual_projections},
...
In Figure \ref{fig:barycenter_vs_group_average}.A, we do so using brain maps taken from the test set.
...
We also display in Figure \ref{fig:barycenter_vs_group_average}.B
...
as shown in Figure \ref{fig:barycenter_vs_group_average}.C.
\begin{figure}[t]
    \centering
    ...
    \caption{
        ...
        We study the same statistical map as in Figure \ref{fig:intro}, which contrasts
